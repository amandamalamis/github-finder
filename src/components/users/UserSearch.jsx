...
function UserSearch() {
    ...
    const handleSubmit = (e) =>
        e.preventDefault()
    if (text === "") {
        setAlert("Enter text to search.", "error")
    }
    else {
        searchUsers(text) //function in context
        setText("")
    }
                ...
                <div>
                    <form onSubmit={handleSubmit}>
                        <div className="form-control">
                            ...
                            <button onClick={clearUsers} type="submit" className="absolute top-0 right-0 rounded-l-none w-36 btn btn-lg">GO</button>
                        </div>
                    </form>
                </div>
                {users.length > 0 && (
